...
public class ReckonDao {
    ...
    protected void addToBudget(Reckon reckon) {
        ...
        try {
            String sql = String.format("INSERT INTO budget.expenses (type, description, amount, date) VALUES('%s', '%s', %.2f, '%tT')",
                    reckon.getType(), reckon.getDescription(), reckon.getAmount(), reckon.getDate());
    ...
    protected boolean updateRow(Reckon reckon) {
        ...
        try {
            ...
            preparedStatement.setDouble(3, reckon.getAmount());
